...
typedef volatile struct sdhci {
	...
	uint32_t pad5;
	uint32_t slot_int;
} __attribute__((packed, aligned(4))) sdhci;


typedef struct sdhci_resp {
	uint32_t response[4];
	uint32_t command;
} sdhci_resp;


//sdhci->trx_mode fields
...
#define RESP_R1b (INDEX_CHECK | CRC_ENABLE | RESP_48b)
...
extern const struct blk_dev blk_sdhci;
